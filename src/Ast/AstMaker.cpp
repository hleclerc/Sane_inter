...
AstMaker::AstMaker( ErrorList *error_list ) : error_list( error_list ) {
    global_inst    = false;
    export_inst    = false;
    static_inst    = false;
    mixin_inst     = false;
...
void AstMaker::parse( const char *content, const char *sourcefile, bool display_lexems ) {
    // src => lexem tree
    Lexer lx( error_list );
    ...
    while ( ! aw.ql.empty() ) {
        AstWriter::Waiting p = aw.ql.front();
        aw.ql.pop_front();
        *p.first = aw.bs.size();
...
void AstMaker::make_single( const Lexem *l ) {
    if ( ! l )
...
Vec<const Lexem *> AstMaker::make_lex_vec_from( const Lexem *l, const Lexem *end ) {
    Vec<const Lexem *> res;
    if ( l ) {
...
void AstMaker::add_error( const std::string &msg, const Lexem *l ) {
    ErrorList::Error &e = error_list->add( msg ).ac( l->beg_src, l->o_beg, l->src );
    std::cerr << e;
}

Hpipe::CmString AstMaker::inheritance_name( size_t i ) {
...
const Lexem *AstMaker::parent_class( const Lexem *lex ) {
    while( lex->prev )
        lex = lex->prev;
    while ( lex->parent ) {
        lex = lex->parent;
...
void AstMaker::write_to_stream( std::ostream &os ) const {
...
AstCrepr AstMaker::to_AstCrepr() const {
    // code
    RcString code( aw.cq.size() );
...
void AstMaker::make_ast_variable( const Lexem *lex ) {
...
void AstMaker::make_ast_number( const Lexem *lex ) {
    aw.write_number( lex, lex->beg, lex->len );
}

void AstMaker::make_ast_string( const Lexem *lex ) {
    std::string str;
...
void AstMaker::make_ast_paren( const Lexem *l ) {
    // []
    if ( *l->beg == '[' ) {
        Vec<size_t> sizes;
        Vec<const Lexem *> lst;
...
void AstMaker::make_ast_block( const Lexem *lex ) {
    Vec<const Lexem *> vl;
...
void AstMaker::make_ast_assign( const Lexem *lex, int type ) {
...
void AstMaker::make_ast_callable( const Lexem *lex, bool def ) {
    // parsed data
...
void AstMaker::make_ast_enum( const Lexem *lex ) {
...
void AstMaker::make_ast_call_op( const Lexem *lex ) {
    Vec<const Lexem *> args;
    for( int i = 0; i < 2; ++i )
        if ( lex->children[ i ] )
            args << lex->children[ i ];
...
void AstMaker::make_ast_pass( const Lexem *lex ) {
    aw.write_pass();
}

void AstMaker::make_ast_if( const Lexem *lex ) {
...
void AstMaker::make_ast_try( const Lexem *lex ) {
...
void AstMaker::make_ast_while( const Lexem *lex ) {
    aw.write_while( lex->children[ 0 ], make_lex_vec_from( lex->children[ 1 ] ) );
}

void AstMaker::make_ast_static( const Lexem *lex ) {
    auto r = raii_save( static_inst, true );
    make_single( lex->children[ 0 ] );
}

void AstMaker::make_ast_mixin( const Lexem *lex ) {
    auto r = raii_save( mixin_inst, true );
    make_single( lex->children[ 0 ] );
}

void AstMaker::make_ast_const( const Lexem *lex ) {
    auto r = raii_save( const_inst, true );
    make_single( lex->children[ 0 ] );
}

void AstMaker::make_ast_export( const Lexem *lex ) {
    auto r = raii_save( export_inst, true );
    make_single( lex->children[ 0 ] );
}

void AstMaker::make_ast_global( const Lexem *lex ) {
    auto r = raii_save( global_inst, true );
    make_single( lex->children[ 0 ] );
}

void AstMaker::make_ast_virtual( const Lexem *lex ) {
    auto r = raii_save( virtual_inst, true );
    make_single( lex->children[ 0 ] );
}
void AstMaker::make_ast_override( const Lexem *lex ) {
    auto r = raii_save( override_inst, true );
    make_single( lex->children[ 0 ] );
}
void AstMaker::make_ast_inline( const Lexem *lex ) {
    auto r = raii_save( inline_inst, true );
    make_single( lex->children[ 0 ] );
}
void AstMaker::make_ast_abstract( const Lexem *lex ) {
    auto r = raii_save( abstract_inst, true );
    make_single( lex->children[ 0 ] );
}

void AstMaker::make_ast_for( const Lexem *lex ) {
...
void AstMaker::make_ast_import( const Lexem *lex ) {
    Vec<Hpipe::CmString> names;
...
void AstMaker::make_ast_lambda( const Lexem *lex ) {
...
void AstMaker::make_ast_get_attr( const Lexem *lex, bool ptr, bool ask, bool ddo ) {
    ASSERT( ask == false, "..." );
...
void AstMaker::make_ast_init_of( const Lexem *lex ) {
...
void AstMaker::make_ast_and( const Lexem *lex ) {
    return aw.write_and( lex, lex->children[ 0 ], lex->children[ 1 ] );
}

void AstMaker::make_ast_or( const Lexem *lex ) {
    return aw.write_or( lex, lex->children[ 0 ], lex->children[ 1 ] );
}


void AstMaker::make_ast_alternat( const Lexem *lex ) {
...
void AstMaker::make_var_assembly( const Lexem *lex ) {
    Vec<const Lexem *> ch;
...
void AstMaker::make_ast_apply( const Lexem *lex, const std::function<void (const Lexem *, const Vec<const Lexem *> &, const Vec<Hpipe::CmString> &, const Vec<size_t> &)> &cb ) {
    // private/protected
...
void AstMaker::make_ast_block( const Vec<const Lexem *> &vl ) {
    aw.write_block( vl );
}
